...
class EnhancedOddspediaScraper:
    ...
    async def scrape_max_odds(self) -> Optional[Dict[str, Any]]:
        """Scrape max odds data."""
        start_date, end_date = self.get_current_date_range()
        
        url = f"https://oddspedia.com/api/v1/getMaxOddsWithPagination?geoCode=IN&bookmakerGeoCode=IN&bookmakerGeoState=&wettsteuer=0&startDate={quote(start_date)}&endDate={quote(end_date)}&sport=football&ot=100&excludeSpecialStatus=0&popularLeaguesOnly=0&sortBy=default&status=all&page=1&perPage=50&inplay=1&language=en"
